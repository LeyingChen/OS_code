...
static int start[3] = {0x10000, 0x20000, 0x30000};
...
int main(int argc, char *argv[]){
	...
	FILE *bootp, *kernelp, *imagep, *process1p, *process2p, *process3p;
	int num_sec;
	imagep = fopen("./image", "w+");
	assert(imagep);

	boot_phdr = read_exec_file(&bootp, argv[argc-5], &boot_ehdr);
	write_bootblock(&imagep, bootp, boot_ehdr, boot_phdr);

	kernel_phdr = read_exec_file(&kernelp, argv[argc-4], &kernel_ehdr);
	write_kernel(&imagep, kernelp, kernel_ehdr, kernel_phdr);

	process1_phdr = read_exec_file(&process1p, argv[argc-3], &process1_ehdr);
	write_process(1, &imagep, process1p, process1_ehdr, process1_phdr);
	
	process2_phdr = read_exec_file(&process2p, argv[argc-2], &process2_ehdr);
	...
	num_sec = count_sectors(process3_ehdr, process3_phdr);
